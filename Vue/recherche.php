<?php $titre = "Bar � cocktail - Liste de cocktail "; ?>
...
<div class="flex flex-grow ">
    ...
    <main class="flex mx-auto items-center">
        <?php if(!empty($tab_aliment["voulu"]) || !empty($tab_aliment["non_voulu"])) { ?>
        <?php foreach($classement_cocktail as $id=>$cocktail): ?>
            <div class=" w-60 flex flex-col justify-around p-3 m-6 rounded-lg border-2 border-gray-500 bg-white transition duration-300 transform hover:scale-105 hover:border-black">
                <div class="flex justify-evenly">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class=" ml-3 h-6 w-6 flex-shrink-0" fill="<?=in_array($id,$tab_IdCockatil_favoris)?"red":"none" ?>" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <?php endforeach ?>
        <?php }else {?> <span class="flex mx-auto bg-white rounded border-2 p-2 border-black"><?="Probleme dans votre requete : recherche impossible"?></span><?php }?>
    </main>
</div>
